...
class ConfigParser:
    ...
    def _evaluate_constant_expression(self, line):
        ...
        try:
            # Используем eval с текущим пространством переменных
...
def main():
    ...
    parser.add_argument("-i", "--input", required=True, help="Путь к входному файлу с текстом конфигурации.")
    parser.add_argument("-o", "--output", required=True, help="Путь к выходному файлу TOML.")
    args = parser.parse_args()

    # Читаем входные данные из файла (с кодировкой UTF-8)
    try:
    ...
    # Записываем результат в файл TOML (с кодировкой UTF-8)
    try:
